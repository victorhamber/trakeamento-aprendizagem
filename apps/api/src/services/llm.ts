...
export class LlmService {
  ...
  private fallbackReport(snapshot: any): string {
    ...
    lines.push(`|---|---:|---:|`);
    lines.push(`| Meta | Spend | R$ ${this.formatNumber(m.spend)} |`);
    lines.push(`| Meta | Impressions | ${this.formatInt(m.impressions)} |`);
    lines.push(`| Meta | Clicks | ${this.formatInt(m.clicks)} |`);
    lines.push(`| Meta | CTR (calc) | ${this.formatNumber(d.ctr_calc_pct)}% |`);
    lines.push(`| Meta | CPM (calc) | R$ ${this.formatNumber(d.cpm_calc)} |`);
    lines.push(`| Meta | CPC (calc) | R$ ${this.formatNumber(d.cpc_calc)} |`);
    lines.push(`| Site | Landing Page Views | ${this.formatInt(m.landing_page_views)} |`);
    lines.push(`| Site | Click → LP | ${this.formatNumber(d.click_to_lp_rate_pct)}% |`);
    ...
    lines.push(`1) Confirmar dados do Meta (meta_insights_daily) e janela de ${this.formatInt(snapshot?.period_days)} dias.`);
