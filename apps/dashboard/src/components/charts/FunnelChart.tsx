type FunnelPayload = {
  page_views?: number;
  leads?: number;
  checkouts?: number;
  purchases?: number;
};

/** Cores próximas ao print (teal → azul → roxo) */
const SEG_STOPS: { a: string; b: string; c: string }[] = [
  { a: '#26c6da', b: '#00acc1', c: '#00838f' },
  { a: '#2196f3', b: '#1e88e5', c: '#1565c0' },
  { a: '#7e57c2', b: '#673ab7', c: '#5e35b1' },
  { a: '#6a1b9a', b: '#4a148c', c: '#38006b' },
];

const ICON_RAIL = 50;

function fmtCount(n: number) {
  const v = Number(n || 0);
  if (!Number.isFinite(v)) return '0';
  if (v >= 1_000_000) {
    const x = v / 1_000_000;
    return `${x.toFixed(1).replace('.', ',').replace(/,?0$/, '')}M`;
  }
  if (v >= 10_000) return v.toLocaleString('pt-BR', { maximumFractionDigits: 0 });
  if (v >= 1_000) {
    const k = v / 1_000;
    return `${k.toFixed(1).replace('.', ',')}K`;
  }
  return String(Math.trunc(v));
}

function fmtPctOfTop(part: number, top: number) {
  if (top <= 0) return '0,0%';
  const p = (part / top) * 100;
  return `${p.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}%`;
}

/** Borda esquerda do trapézio no meio vertical do segmento (ponto na diagonal) */
function leftEdgeXAtMid(g: { x0l: number; x1l: number }) {
  return (g.x0l + g.x1l) / 2;
}

/** Início aproximado da área de texto (à direita da coluna de ícones), alinhado à borda esquerda em cima e embaixo */
function contentTextCenter(g: { x0l: number; x0r: number; x1l: number; x1r: number; y0: number; y1: number }, rail: number) {
  const pad = 10;
  const l0 = g.x0l + rail + pad;
  const l1 = g.x1l + rail + pad;
  const topM = (l0 + g.x0r) / 2;
  const botM = (l1 + g.x1r) / 2;
  return (topM + botM) / 2;
}

function FunnelIcon({ kind, x, y }: { kind: 'page' | 'users' | 'cart' | 'bag'; x: number; y: number }) {
  const s = { stroke: 'rgba(255,255,255,0.95)', fill: 'none' as const, strokeWidth: 1.5, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const };
  if (kind === 'page') {
    return (
      <g transform={`translate(${x - 9} ${y - 7})`}>
        <rect x="1" y="3" width="16" height="10" rx="1.5" {...s} />
        <path d="M4 3V2A1.5 1.5 0 0 1 5.5.5H12A1.5 1.5 0 0 1 14 2V3" {...s} />
        <path d="M6.5 8.5h5" stroke="rgba(255,255,255,0.9)" fill="none" strokeWidth="1.2" strokeLinecap="round" />
      </g>
    );
  }
  if (kind === 'users') {
    return (
      <g transform={`translate(${x - 11} ${y - 7})`}>
        <circle cx="6.5" cy="3.8" r="2.5" {...s} />
        <path d="M2.5 14.5v-1A4.5 4.5 0 0 1 6.5 9.5" {...s} />
        <path d="M6.5 9.5a4.5 4.5 0 0 1 4 4v1" {...s} />
        <circle cx="14" cy="3.5" r="2" stroke="rgba(255,255,255,0.9)" fill="none" strokeWidth="1.2" />
        <path d="M15 5.2v.6" stroke="rgba(255,255,255,0.9)" strokeWidth="1" strokeLinecap="round" />
      </g>
    );
  }
  if (kind === 'cart') {
    return (
      <g transform={`translate(${x - 10} ${y - 6})`}>
        <path
          d="M.5.5H2.5L3.3 5.5A1.2 1.2 0 0 0 4.4 6.5H14.5L15.3 3.5H3.8"
          stroke="rgba(255,255,255,0.95)"
          fill="none"
          strokeWidth="1.5"
          strokeLinejoin="round"
        />
        <circle cx="5.5" cy="13" r="1" fill="rgba(255,255,255,0.2)" stroke="white" strokeWidth="0.7" />
        <circle cx="12.5" cy="13" r="1" fill="rgba(255,255,255,0.2)" stroke="white" strokeWidth="0.7" />
      </g>
    );
  }
  return (
    <g transform={`translate(${x - 7.5} ${y - 6.5})`}>
      <path d="M2.5 2.5H13.5A1.2 1.2 0 0 1 15 3.6V12l-0.8 1.2H3L2 12V3.6A1.2 1.2 0 0 1 2.5 2.5z" {...s} />
      <path d="M6.5 2.5V1.3A.7.7 0 0 1 7.1.7H9.2a.7.7 0 0 1 .7.7V2.5" {...s} />
    </g>
  );
}

export function FunnelChart({ data, isDark }: { data: FunnelPayload | null; isDark: boolean }) {
  if (!data) return null;

  const page = Number(data.page_views || 0);
  const lead = Number(data.leads || 0);
  const ic = Number(data.checkouts || 0);
  const pur = Number(data.purchases || 0);
  const top = page;

  const stages = [
    { key: 'page', label: 'PageView', value: page, icon: 'page' as const },
    { key: 'lead', label: 'Lead', value: lead, icon: 'users' as const },
    { key: 'ic', label: 'InitiateCheckout', value: ic, icon: 'cart' as const },
    { key: 'pur', label: 'Compras', value: pur, icon: 'bag' as const },
  ];

  const W = 900;
  const topY = 8;
  const stageH = 108;
  const gap = 6;
  const skew = 16;
  const widths = [1, 0.84, 0.68, 0.5];

  const totalStagesH = 4 * stageH + 3 * gap;
  const H = topY + totalStagesH + 16;
  const leftX = 56;
  const rightX = W - 56;

  const layer = (i: number) => {
    const y0 = topY + i * (stageH + gap);
    const y1 = y0 + stageH;
    const w0 = (rightX - leftX) * widths[i];
    const w1 = (rightX - leftX) * (widths[i + 1] ?? widths[3] * 0.82);
    const cx = W / 2;
    const x0l = cx - w0 / 2;
    const x0r = cx + w0 / 2 + skew;
    const x1l = cx - w1 / 2;
    const x1r = cx + w1 / 2 + skew;
    return { y0, y1, x0l, x0r, x1l, x1r, cx, midY: (y0 + y1) / 2 };
  };

  const labelFill = isDark ? 'rgba(255,255,255,0.88)' : 'rgba(15,23,42,0.6)';
  const valueFill = isDark ? '#ffffff' : '#0f172a';
  const pctFill = isDark ? 'rgba(255,255,255,0.95)' : '#0f172a';
  const railFill = isDark ? 'rgba(0,0,0,0.42)' : 'rgba(0,0,0,0.1)';
  const strokePoly = isDark ? 'rgba(255,255,255,0.12)' : 'rgba(15, 23, 42, 0.1)';

  return (
    <div className="w-full select-none outline-none focus:outline-none -mx-1">
      <svg
        viewBox={`0 0 ${W} ${H}`}
        className="w-full h-auto min-h-[380px] block"
        role="img"
        aria-label="Funil: PageView, Lead, InitiateCheckout, Compras"
        preserveAspectRatio="xMidYMin meet"
      >
        <defs>
          {SEG_STOPS.map((st, i) => (
            <linearGradient key={`seg${i}`} id={`funnelSeg${i}`} x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor={st.a} />
              <stop offset="55%" stopColor={st.b} />
              <stop offset="100%" stopColor={st.c} />
            </linearGradient>
          ))}
          {stages.map((_, i) => {
            const g = layer(i);
            const pts = `${g.x0l},${g.y0} ${g.x0r},${g.y0} ${g.x1r},${g.y1} ${g.x1l},${g.y1}`;
            return (
              <clipPath key={`c${i}`} id={`funnelClip${i}`}>
                <polygon points={pts} />
              </clipPath>
            );
          })}
        </defs>

        {stages.map((s, i) => {
          const g = layer(i);
          const pts = `${g.x0l},${g.y0} ${g.x0r},${g.y0} ${g.x1r},${g.y1} ${g.x1l},${g.y1}`;
          const xText = contentTextCenter(g, ICON_RAIL);
          const xPct = Math.max(g.x0r, g.x1r) - 16;
          const longLabel = s.label.length > 14;
          const share = top <= 0 ? '0,0%' : i === 0 ? '100,0%' : fmtPctOfTop(s.value, top);
          const lx = leftEdgeXAtMid(g);
          const railX0 = lx + 2;
          const iconCx = railX0 + (ICON_RAIL - 4) / 2;

          return (
            <g key={s.key}>
              <polygon
                points={pts}
                fill={`url(#funnelSeg${i})`}
                fillOpacity={isDark ? 1 : 0.9}
                stroke={strokePoly}
                strokeWidth="1"
              />
              <g clipPath={`url(#funnelClip${i})`}>
                <rect
                  x={railX0}
                  y={g.y0 + 2}
                  width={ICON_RAIL - 2}
                  height={g.y1 - g.y0 - 4}
                  rx="5"
                  fill={railFill}
                />
                <FunnelIcon kind={s.icon} x={iconCx} y={g.midY} />
              </g>

              <text
                x={xText}
                y={g.midY - 12}
                textAnchor="middle"
                fill={labelFill}
                fontSize={longLabel ? 15.5 : 18}
                fontWeight="600"
                fontFamily="system-ui, 'Segoe UI', Inter, sans-serif"
              >
                {s.label}
              </text>
              <text
                x={xText}
                y={g.midY + 24}
                textAnchor="middle"
                fill={valueFill}
                fontSize="34"
                fontWeight="800"
                fontFamily="system-ui, 'Segoe UI', Inter, sans-serif"
              >
                {fmtCount(s.value)}
              </text>
              <text
                x={xPct}
                y={g.midY + 4}
                textAnchor="end"
                fill={pctFill}
                fontSize="20"
                fontWeight="800"
                fontFamily="system-ui, 'Segoe UI', Inter, sans-serif"
              >
                {share}
              </text>
            </g>
          );
        })}
      </svg>
    </div>
  );
}
